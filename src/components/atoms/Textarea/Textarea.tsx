...
export default function Textarea({
  ...
  const nearLimit = maxChars !== undefined && currentLength !== undefined && currentLength >= maxChars * 0.9;
  ...
  return (
    <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center justify-between">
          ...
          {maxChars !== undefined && currentLength !== undefined && (
            <span
              id={counterId}
              aria-live="polite"
              className={cn("text-xs tabular-nums", nearLimit ? "text-red-500" : "text-gray-400")}
            >
      ...
      <textarea
        id={id}
        aria-describedby={
          [hint && hintId, error && errorId, maxChars !== undefined && counterId].filter(Boolean).join(" ") ||
          undefined
        }
      ...
      {error && (
        <p id={errorId} role="alert" className="text-xs leading-relaxed text-red-500">
          {error}
        </p>
      )}
    </div>

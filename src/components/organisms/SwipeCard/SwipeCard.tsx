...
function UserGroupIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-3.5 w-3.5 text-gray-400">
...
export default function SwipeCard({
  ...
  const cardContent = (
    <div className="flex h-full flex-col">
      <div className="relative h-[120px] flex-shrink-0 overflow-hidden rounded-t-2xl bg-gray-50">
        <div className="absolute -top-6 right-10 h-[110px] w-[150px] rounded-full bg-[#A3FF38]/80 blur-2xl" />
        <div className="absolute top-4 right-20 h-[70px] w-[90px] rounded-full bg-[#A3FF38]/50 blur-xl" />
        {app.audienceSize && (
          <div className="absolute right-4 bottom-3 flex items-center gap-1.5 rounded-full border border-gray-100/80 bg-white px-3 py-1.5 shadow-sm">
            <UserGroupIcon />
      ...
      <div className="flex-shrink-0 px-5" style={{ marginTop: -32 }}>
        {app.avatarUrl ? (
          ...
          <div
            className={`flex h-[72px] w-[72px] items-center justify-center rounded-full border-[3px] border-white text-xl font-bold shadow-md ${colorClass}`}
          >
            {initials}
          </div>
        )}
      ...
      <div className="flex-shrink-0 px-5 pt-2.5">
        <p className="text-[18px] leading-tight font-bold text-neutral-900">{app.displayName}</p>
        <p className="mt-0.5 text-[13px] text-gray-400">{username}</p>
        {app.bio && <p className="mt-2 line-clamp-3 text-[13px] leading-relaxed text-gray-500">{app.bio}</p>}
      </div>

      {app.niches.length > 0 && (
        <div className="flex-shrink-0 px-5 pt-3">
          <p className="mb-2 text-[13px] font-semibold text-neutral-900">Niches</p>
          <div className="flex flex-wrap gap-1.5">
            {app.niches.slice(0, 5).map((n) => (
              <span key={n} className="rounded-lg border border-gray-200 px-3 py-1 text-[12px] font-medium text-neutral-600">
                {n}
              </span>
            ))}
          </div>
      ...
      {socials.length > 0 && (
        <div className="flex-shrink-0 px-5 pt-3 pb-5">
          <p className="mb-2 text-[13px] font-semibold text-neutral-900">Socials</p>
          <div className="flex flex-wrap gap-2">
            {socials.map((s) => (
              <span
                key={s.platform}
                className="flex items-center gap-2 rounded-lg border border-gray-200 px-2.5 py-1.5 text-[12px] font-medium text-neutral-700"
              >
                <span
                  className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md text-white ${PLATFORM_BG[s.platform] ?? "bg-gray-400"}`}
                >
  ...
  if (isGhost) {
    return (
      <div className="h-full w-full overflow-hidden rounded-2xl border border-gray-100/80 bg-white shadow-lg">
        {cardContent}
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 h-full w-full cursor-grab overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-2xl select-none active:cursor-grabbing"
      style={{ x, rotate, touchAction: "pan-y" }}
    >
      <motion.div
        className="pointer-events-none absolute inset-0 z-10 flex items-center justify-start rounded-2xl pl-8"
        style={{ opacity: approveOpacity, background: "rgba(163,255,56,0.85)" }}
      >
        <div className="flex flex-col items-center gap-1">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-14 w-14 text-neutral-900 drop-shadow-sm">
            <path
              fillRule="evenodd"
          ...
          <span className="text-[13px] font-bold tracking-wide text-neutral-900 uppercase">Approve</span>
        </div>
      </motion.div>

      <motion.div
        className="pointer-events-none absolute inset-0 z-10 flex items-center justify-end rounded-2xl pr-8"
        style={{ opacity: denyOpacity, background: "rgba(239,68,68,0.85)" }}
      >
        <div className="flex flex-col items-center gap-1">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-14 w-14 text-white drop-shadow-sm">
            <path
              fillRule="evenodd"
          ...
          <span className="text-[13px] font-bold tracking-wide text-white uppercase">Pass</span>
        </div>
      </motion.div>

      <div className="relative z-0 h-full">{cardContent}</div>
    </motion.div>
  );
}

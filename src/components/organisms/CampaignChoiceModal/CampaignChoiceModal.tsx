...
export default function CampaignChoiceModal({ shop, onClose, onChat, onPayWithPost }: CampaignChoiceModalProps) {
  ...
  const { label } = resolveContentType(shop.tier);
  const article = /^([aeiou]|x )/i.test(label) ? "an" : "a";
          ...
          <div className="relative rounded-[24px] bg-white p-6 sm:p-7">
            ...
            <div className="pr-10">
              ...
              <p className="mt-3 max-w-md text-sm leading-6 text-neutral-500">
                {`Create ${article} ${label} for ${shop.product}, then send it to ${shop.brand} for approval by ${shop.deadline}.`}
              </p>
            </div>
